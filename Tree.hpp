...
*/
              ...
              void load_data(float **_input_, int *labels, int height, int width, int _number_, string *feature_names){
                  data = cv::Mat(height, width, CV_64FC1);
                  ...
                  label = cv::Mat(height, 1, CV_64FC1);
                  for(int i = 0; i < height; i++){
              ...
              float Gini(cv::Mat *_data_, cv::Mat *_label_, int index, int *return_counts);
              ...
              void Predict(cv::Mat *_data_, cv::Mat *_label_);
